...
namespace petuum {
  ...
  size_t ThreadTable::IndexUpdateAndGetCount(int32_t row_id,
                                             size_t num_updates) {
    int32_t partition_num = GlobalContext::GetPartitionCommChannelIndex(row_id);
    oplog_index_[partition_num].insert(row_id);
  ...
  void ThreadTable::InsertRow(int32_t row_id,
  ...
  void ThreadTable::Inc(int32_t row_id,
                        int32_t column_id,
                        const void *delta) {
  ...
  void ThreadTable::BatchInc(int32_t row_id,
  ...
  void ThreadTable::DenseBatchInc(int32_t row_id,
                                  const void *updates,
                                  int32_t index_st,
                                  int32_t num_updates) {
  ...
  } // end function -- ApplyThreadOpLogSSP
