...
namespace petuum {
  ...
  public:
    ...
    // exclusive access
    virtual int32_t Inc(int32_t row_id,
                        int32_t column_id,
                        const void *delta) = 0;

    virtual int32_t BatchInc(int32_t row_id,
                             const int32_t *column_ids,
                             const void *deltas,
                             int32_t num_updates) = 0;

    virtual int32_t DenseBatchInc(int32_t row_id,
                                  const void *updates,
                                  int32_t index_st,
                                  int32_t num_updates) = 0;

    // Guaranteed exclusive accesses to the same row id.
    virtual bool FindOpLog(int32_t row_id,
    ...
    // row oplog exists.
    virtual bool FindAndLock(int32_t row_id,
                             OpLogAccessor *oplog_accessor) = 0;
    ...
    virtual bool GetEraseOpLogIf(int32_t row_id,
