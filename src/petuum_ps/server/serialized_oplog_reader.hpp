...
namespace petuum {
      ...
      while(1) {
        ...
        if (num_rows_left_in_current_table_ > 0) {
          ...
          *table_id = current_table_id_;
          ...
          // 2. parse new row id
          *row_id = *(reinterpret_cast<const int32_t*>(serialized_oplog_ptr_ + offset_));
          offset_ += sizeof(int32_t);
          ...
          const void *update = GetNextUpdate_(curr_sample_row_oplog_,
  ...
  private:
    typedef const void *(*GetNextUpdateFunc)(const AbstractRowOpLog* sample_row_oplog,
                                             const void *mem,
    ...
    static const void *GetNextUpdateSparse(const AbstractRowOpLog* sample_row_oplog,
                                           const void *mem,
                                           int32_t const ** column_ids,
                                           int32_t *num_updates,
                                           size_t *serialized_size) {
      return sample_row_oplog->ParseSparseSerializedOpLog(mem,
                                                          column_ids,
                                                          num_updates,
                                                          serialized_size);
    }
    ...
    static const void *GetNextUpdateDense(const AbstractRowOpLog* sample_row_oplog,
                                          const void *mem,
                                          int32_t const ** column_ids,
                                          int32_t *num_updates,
                                          size_t *serialized_size) {

      return sample_row_oplog->ParseDenseSerializedOpLog(mem,
                                                         num_updates,
                                                         serialized_size);
    }
    ...
    void StartNewTable() {
      ...
      current_table_id_ = *(reinterpret_cast<const int32_t*>(serialized_oplog_ptr_ + offset_));
      offset_ += sizeof(int32_t);

      update_size_ = *(reinterpret_cast<const size_t*>(serialized_oplog_ptr_ + offset_));
      offset_ += sizeof(size_t);

      num_rows_left_in_current_table_ = *(reinterpret_cast<const int32_t*>(serialized_oplog_ptr_ + offset_));
      offset_ += sizeof(int32_t);
