#include <petuum_ps/server/replica_thread.hpp>
...
namespace petuum {
  ...
  void ReplicaThread::ConnectToNameNode() {
  ...
  void ReplicaThread::ConnectToScheduler() {
  ...
  int32_t ReplicaThread::GetConnection(bool *is_client, bool *is_aggregator, int32_t *client_id) {
    int32_t sender_id;
    zmq::message_t zmq_msg;
    ...
    if (msg_type == kClientConnect) {
      ...
      *is_client = true; *is_aggregator = false;
  ...
  void ReplicaThread::InitReplica() {
    ...
    int32_t num_bgs = 0;
    int32_t num_aggs = 0; int32_t num_servers = 0;
    ...
    for (num_connections = 0; num_connections < num_expected_connections; ++num_connections) {
      int32_t client_id;
      bool is_client; bool is_aggregator;
      int32_t sender_id = GetConnection(&is_client, &is_aggregator, &client_id);
      if(is_client) {
