...
namespace petuum {
  ...
  public:
    ...
    explicit ServerTable(const TableInfo &table_info):
      ...
      if (table_info.row_oplog_type == RowOpLogType::kDenseRowOpLog) {
        sample_row_oplog_ = new DenseRowOpLog(InitUpdateFunc(),
                                              CheckZeroUpdateFunc(),
                                              sample_row_->get_update_size(),
                                              table_info.dense_row_oplog_capacity);
      }
      else {
        sample_row_oplog_ = new SparseRowOpLog(InitUpdateFunc(),
                                               CheckZeroUpdateFunc(),
                                               sample_row_->get_update_size());
      }
  ...
  private:
    ...
    static void ApplyRowBatchInc(const int32_t *column_ids,
                                 const void *updates,
                                 int32_t num_updates,
    ...
    typedef void (*SortCandidateVectorFunc)(std::vector<CandidateServerRow> *candidate_row_vector);
