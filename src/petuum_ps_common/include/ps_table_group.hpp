...
namespace petuum {
  ...
  public:
...
#ifdef PETUUM_SINGLE_NODE
      abstract_table_group_ = new TableGroupSN(table_group_config, table_access, &init_thread_id);
#else
      abstract_table_group_ = new TableGroup(table_group_config, table_access, &init_thread_id);
#endif
    ...
    static void RegisterRow(int32_t row_type) {
      ClassRegistry<AbstractRow>::GetRegistry().AddCreator(row_type, CreateObj<AbstractRow, ROW>);
    }
    ...
    static Table<UPDATE> GetTableOrDie(int32_t table_id) {
      AbstractClientTable *abstract_table = abstract_table_group_->GetTableOrDie(table_id);
